...
export class CircleVertex extends VertexShader {
    ...
    execute(args) {
        ...
        for (let i = 0; i < Math.ceil(Math.PI * radius * 64); i++) {
            ret.push(new BlockLocation(pt1.x + Math.round(radius * Math.sin(step * i * RADIAN)), pt1.y, pt1.z + Math.round(radius * Math.cos(step * i * RADIAN))));
        }
        return ret;
    }
}
...
export class BaseFragment extends Shader {
    ...
    execute(args) {
        ...
        for (let i of vertex) {
            let blockData = _MinecraftBlockTypes[block].createDefaultBlockPermutation();
            ret.push(blockData);
        }
        return ret;
    }
...
export class UnweightedBlockDistFragment extends Shader {
    ...
    execute(args) {
        ...
        for (let i of vertex) {
            let blockData = _MinecraftBlockTypes[block[Math.floor(Math.random() * (block.length))]].createDefaultBlockPermutation();
            ret.push(blockData);
        }
        return ret;
    }
